...
class SimulatorBase:
    ...
    def __init__(self, config:BaseConfig):
        ...
        self.camera_poses = {}
        self.default_pose = {
            "position": np.array([0, 0.0, 1.0]),  # defalut position
            "orientation": np.array([0, 0, 0, 1]),  # defalut orientation
            "name" : "mobilebase0_top_camera"
        }
        ...
        self.record_start_sub = rospy.Subscriber('/start', Bool, self.start_record_callback)
        ...
        # camera_names = ["top_camera", "wrist_cam_left", "wrist_cam_right"]
    ...
    def camera_pose_callback(self, msg):
        ...
        if 'top' in msg.camera_name.lower():
            ...
            delta_rot = Rotation.from_euler('x', 0, degrees=True)
    ...
    def save_to_hdf5(self, data_dict, dataset_dir, episode_idx, camera_names):
        ...
        with h5py.File(dataset_path, 'w', rdcc_nbytes=1024**2 * 2) as root:
            ...
            for cam_name in camera_names:
                img_data = np.array(data_dict['observations']['images'][cam_name][10:])
            ...
            # 保存关节位置 (qpos)
            qpos_data = np.array(data_dict['observations']['qpos'][10:])
            obs.create_dataset('qpos', data=qpos_data)
            
            # 保存动作 (actions)
            action_data = np.array(data_dict['observations']['actions'][10:])
    ...
    def record_frame(self, data_dict, left_arm_actions, right_arm_actions, frame_top, frame_wrist_left, frame_wrist_right):
        ...
        # 记录动作
        data_dict['observations']['actions'].append(qpos)
        ...
        data_dict['observations']['images'][top_camera_name].append(frame_top)
        data_dict['observations']['images'][wrist_left_camera_name].append(frame_wrist_left)
        data_dict['observations']['images'][wrist_right_camera_name].append(frame_wrist_right)
    # @abstractmethod
    ...
    def post_physics_step(self):
        ...
        def tile_images(images, rows, cols):
            ...
            top_index = 1
            right_index = 2
            # if len(images) > left_index:
            #     canvas[0:img_h, 0:img_w, :] = images[left_index]
            # if len(images) > top_index:
            #     canvas[0:img_h, img_w:2 * img_w, :] = images[top_index]
            # if len(images) > right_index:
            #     canvas[0:img_h, 2 * img_w:3 * img_w, :] = images[right_index]
        ...
        if frame_wrist_left is not None:
            self.all_images.append(cv2.cvtColor(frame_wrist_left, cv2.COLOR_BGR2RGB))
        if frame_top is not None:
            self.all_images.append(cv2.cvtColor(frame_top,  cv2.COLOR_BGR2RGB))
        if frame_wrist_right is not None:
            self.all_images.append(cv2.cvtColor(frame_wrist_right,cv2.COLOR_BGR2RGB) )

        if fix is not None:
            self.all_images.append(cv2.cvtColor(fix,cv2.COLOR_BGR2RGB) )
        if fix_2 is not None:
            self.all_images.append(cv2.cvtColor(fix_2,cv2.COLOR_BGR2RGB) )
        ...
        if self.record_flag == True:
            print("is done episode true")
            # camera_names_record = ["top_camera", "wrist_cam_left", "wrist_cam_right"]
            camera_names_record = ["mobilebase0_top_camera", "robot0_wrist_cam_left", "robot0_wrist_cam_right"]
            # dataset_dir = "/home/jiziheng/Music/robot/gs_scene/mobile_ai_gs/data/hdf5_1"

            # dataset_dir = "/home/cfy/cfy/cfy/gs/mobile_ai_gs/data/hdf5_2"
            dataset_dir = "/home/cfy/cfy/cfy/gs/mobile_ai_gs/data/0429_apple_iii"
            # if os.path.exists(dataset_dir):
            #     shutil.rmtree(dataset_dir)  # 递归删除目录
            #     os.makedirs(dataset_dir)    # 重新创建空目录
        ...
        canvas_img = tile_images(self.all_images, rows=2, cols=3)
        ...
        if canvas_img is not None:
            ...
            rospy.logwarn_once("canvas_img is None")
